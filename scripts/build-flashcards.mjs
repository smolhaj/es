...
async function loadFrequency() {
  ...
  for await (const line of rl) {
    ...
    // The source corpus itself flags rows it has no real usage evidence
    // for (e.g. "fué"/"dió"/"vió" — pre-1952-reform accented spellings
    // that only ever surface as this placeholder row, pos "none"). Their
    // modern spellings ("fue"/"dio"/"vio") aren't separately lemmatized in
    // this corpus at all, so skipping the NOUSAGE row loses nothing.
    if (row.flags === 'NOUSAGE') continue;
...
function loadDictionary() {
  ...
  // A gloss like "nonstandard form of "se"" or "archaic spelling of
  // "Adán"" is a bare cross-reference to a DIFFERENT headword's spelling —
  // never the actual word's meaning — regardless of which of "form"/
  // "spelling" and which qualifier (obsolete/archaic/dated/rare/informal/
  // nonstandard/eye dialect/pronunciation/superseded/misspelling) it uses.
  // A literal string list here undercounts real combinations one at a time
  // (this already missed "archaic form of" and "nonstandard form of" once
  // each, letting texting-slang "c" and the archaic "Adam" spelling slip
  // through as if they were real standalone words) — match the shape
  // instead of enumerating every phrase.
  const BAD_GLOSS_PREFIX_RE = /^(obsolete|archaic|dated|rare|informal|nonstandard|eye dialect|pronunciation|superseded)\s+(form|spelling)\s+of\b|^(alternative form of|misspelling of)/i;
  ...
  for (const block of blocks) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (match && currentPos) {
        ...
        gloss = gloss.split('\\n')[0].trim();
        if (BAD_GLOSS_PREFIX_RE.test(gloss)) continue;
...
// "surname" alone. "^letter:" catches the other common phrasing for a
// letter-name headword ("te" -> "letter: t"), which "^the letter" alone
// misses — that bug shipped a top-25-frequency pronoun ("te") as if it
// only meant the name of the letter T.
// "Dependent preposition following certain verbs" is a usage NOTE, not a
// translation — it carries zero standalone meaning, yet nothing above
// catches it, so it can win a slot in the 2-gloss combine right next to
// "con"'s real "with" sense and ship as "with, Dependent preposition
// following certain verbs" on one of the ~15 most common words in the
// language.
const USELESS_GLOSS = /surname|given name|^the letter\b|^letter:|initialism of|abbreviation of|@abbrev|dependent preposition|following certain (verbs?|nouns?|adjectives?)/i;

// A gloss that's pure unexpanded Wiktionary template markup (the template
// wasn't rendered to plain text by this dataset's own build, e.g.
// "{{es-superseded spelling of|fue|1952}}") teaches nothing and must never
// reach a flashcard verbatim.
function hasRawMarkup(gloss) {
  return gloss.includes('{{') || gloss.includes('}}');
}

// "inflection of "tú,vos": to you, for you" is Wiktionary's own notation
// for an inflected pronoun/verb form — the real meaning lives after the
// colon. "(see usage notes) which one, what, which" is the same shape with
// a different cross-reference marker instead of a colon. Strip both kinds
// of scaffolding so the flashcard shows plain English, not raw dictionary
// notation or a dead pointer to notes the flashcard can't display.
function stripInflectionPrefix(gloss) {
  return gloss
    .replace(/^inflection of "[^"]*":\s*/i, '')
    .replace(/^\(see (the )?usage notes?\)\s*/i, '')
    .trim();
}

// Bare grammatical cross-references ("second-person singular voseo
// imperative of ir", "plural of gato") carry no standalone meaning at all
// — contrast the "inflection of "X": meaning" pattern above, which always
// has real content after its colon. Only fires when there's no colon, so
// it can't catch a legitimate gloss that happens to mention a grammar term
// as part of an actual definition.
const GRAMMAR_CROSS_REF_RE = /\b(imperative|subjunctive|indicative|participle|gerund|preterite|plural|feminine|masculine)\s+of\b/i;
function isBareGrammarCrossRef(gloss) {
  return GRAMMAR_CROSS_REF_RE.test(gloss) && !gloss.includes(':');
}

// Try candidate dictionary blocks for a headword in priority order until
// one produces a usable (non-empty) translation, instead of committing to
// a single "best" block (matched purely by pos code) and dropping the
// whole word outright if that one block's own gloss(es) turn out to be
// junk. That used to be a real failure mode: "te" (a top-25-frequency
// pronoun) has a "pos: n" block whose only gloss is "letter: t" sitting
// right next to its real "pos: pron" block — matching on pos code alone
// picked the letter-name block and either showed it verbatim (before the
// USELESS_GLOSS fix above) or, once that gloss got filtered, would have
// silently dropped "te" from the deck entirely with no fallback. Preference
// order: the block matching frequency.csv's reported pos, then any other
// non-proper-noun block, then proper-noun blocks last — a common-word
// sense is almost always more useful to a learner than an incidental
// capitalized-name homograph (see the getEntriesForRow prop/lowercase
// comment in main() for the matching bug on that side).
function resolveTranslation(entries, wantedPos) {
  const usable = entries.filter(e => !USELESS_POS.has(e.pos));
  if (!usable.length) return null;
  const exact = usable.filter(e => e.pos === wantedPos);
  const nonProp = usable.filter(e => e.pos !== wantedPos && e.pos !== 'prop');
  const prop = usable.filter(e => e.pos !== wantedPos && e.pos === 'prop');
  const tried = new Set();
  for (const cand of [...exact, ...nonProp, ...prop]) {
    const key = cand.pos + ' ' + (cand.gender ?? '');
    if (tried.has(key)) continue;
    tried.add(key);
    const translation = buildTranslation(entries, cand.pos, cand.gender);
    if (translation) return { pos: cand.pos, gender: cand.gender, translation };
  }
  return null;
}
...
function buildTranslation(entries, pos, gender) {
  ...
  for (const e of sameBlock) {
    ...
    if (!g) continue;
    if (hasRawMarkup(g) || isBareGrammarCrossRef(g)) continue;
    g = stripInflectionPrefix(g);
    if (!g) continue;
...
async function main() {
  ...
  // freq is exact-case-keyed (same Map dedup key as dict), so a lowercase
  // and capitalized spelling of the same word can both be separate rows —
  // used just below to tell "this common word already gets its own,
  // correctly-tagged card elsewhere" from "this is its only chance".
  const freqWords = new Set(freq.map(r => r.spanish));
  ...
  for (const row of freq) {
    ...
    let wantedPos = row.pos;
    let entries = dict.get(spanish);

    // Corpus tagging quirk: a capitalized proper-noun lemma sometimes
    // absorbs the vast majority of an unrelated common lowercase word's
    // occurrences — Estado/estado, Liga/liga, Franco/franco, Cordero/
    // cordero, Vega/vega, Moreno/moreno all show this. E.g. the "Estado"
    // lemma's own usage breakdown attributes all 392,772 of its counted
    // occurrences to the surface form "estado" (lowercase) even though the
    // lemma itself is tagged 'prop' — that's the ordinary word "estado"
    // ("state/status", also the past participle of "estar") getting folded
    // into an incidental capitalized-name homograph, not people actually
    // capitalizing it. Prefer the lowercase headword's ordinary senses
    // when one exists with real content.
    if (wantedPos === 'prop' && spanish !== spanish.toLowerCase()) {
      const lower = spanish.toLowerCase();
      const lowerEntries = dict.get(lower);
      const usableLower = lowerEntries?.find(e => e.pos !== 'prop' && !USELESS_POS.has(e.pos));
      if (usableLower) {
        // If the lowercase word is ALSO its own separate frequency-list
        // row, it gets (or already got) its own correctly-tagged card on
        // its own merits — this capitalized row is then pure redundant
        // noise inherited from the tagging bug above, so drop it rather
        // than ship a near-duplicate card.
        if (freqWords.has(lower)) continue;
        spanish = lower;
        entries = lowerEntries;
        wantedPos = usableLower.pos;
      }
    }

    if (!entries || !entries.length) continue;
    const resolved = resolveTranslation(entries, wantedPos);
    if (!resolved) continue;
    const { pos: bestPos, gender: bestGender, translation } = resolved;
    const posLabel = POS_LABEL[bestPos] ?? bestPos ?? '';
    ...
    const rawSent = sentIndex.get(spanish);
    let sent = rawSent && !rawSent.risky ? rawSent : null;
    if (sent && STRICT_LITERAL_POS.has(posLabel) && !containsLiteralWord(sent.es, spanish)) {
      sent = null;
    }
    ...
    if (sent && posLabel === 'noun' && sentenceGenderConflict(sent.es, spanish, bestGender)) {
      sent = null;
    }
